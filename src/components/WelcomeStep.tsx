...
//
// Sized to fit one screen without scrolling on typical phones - hero
// height, gaps, and icon/text sizes are all deliberately tight rather than
// the more generous spacing used elsewhere, since this is a single
// transitional screen, not a scrollable page.
const FEATURES = [
...
export function WelcomeStep({ onNext }: { onNext: () => void }) {
  return (
    <div className="page-enter relative h-svh overflow-y-auto">
      <img src="/welcome-bg.jpg" alt="" className="fixed inset-0 -z-10 h-full w-full object-cover" />

      <div className="relative mx-auto flex h-full w-full max-w-sm flex-col items-center justify-center gap-3 px-6 pb-[max(1rem,env(safe-area-inset-bottom))] pt-[max(1rem,env(safe-area-inset-top))] text-center">
        ...
        <div className="relative flex min-h-[22vh] w-full flex-col items-center justify-end">
          <div className="relative">
            <div className="absolute bottom-2 left-1/2 h-5 w-24 -translate-x-1/2 rounded-full bg-purple-950/40 blur-xl" />
            <img
              src="/mascot-guide.png"
              alt="Ming"
              className="relative h-36 w-auto object-contain drop-shadow-[0_10px_18px_rgba(0,0,0,0.5)]"
            />
          </div>
        </div>
        ...
        <div className="flex w-full flex-wrap justify-center gap-2 rounded-2xl border border-zinc-800/80 bg-zinc-950/60 p-3 backdrop-blur-sm">
          {FEATURES.map((f) => (
            <div key={f.label} className="flex w-[28%] min-w-[5.5rem] flex-col items-center gap-0.5 text-center">
              <img src={f.icon} alt="" className="h-8 w-8 object-contain" />
              <span className="text-[11px] font-semibold text-white">{f.label}</span>
              <span className="text-[9px] leading-tight text-zinc-400">{f.desc}</span>
            </div>
          ))}
        </div>

        <div className="flex w-full items-center gap-2 rounded-2xl border border-zinc-800/80 bg-zinc-950/60 px-3 py-2 text-left backdrop-blur-sm">
        ...
        <p className="text-[11px] text-zinc-500">Next: Choose your username</p>
      </div>
    </div>
  )
}
