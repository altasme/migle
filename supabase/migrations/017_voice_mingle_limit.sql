-- 5 free voice matches per day per user. Reset is lazy (compare stored
-- date to current_date) rather than a cron job — simplest thing that
-- works at this scale.
alter table profiles
  add column if not exists voice_mingles_used int not null default 0,
  add column if not exists voice_mingles_date date not null default current_date;

-- Client-safe: only ever reports the CALLER's own remaining count, never
-- takes a uuid argument, so it can't be used to probe anyone else's usage.
create or replace function my_voice_mingles_remaining()
returns int language sql security definer stable as $$
  select case
    when voice_mingles_date = current_date then greatest(0, 5 - voice_mingles_used)
    else 5
  end
